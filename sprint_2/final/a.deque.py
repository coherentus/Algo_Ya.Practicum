# https://contest.yandex.ru/contest/22781/run-report/65145009/
# Задача А. Дек.
...
#
# -- ПРИНЦИП РАБОТЫ --
# Если за ключевое условие задания взять временную сложность,
...
#
# Далее, на каждый момент времени достаточно знать положение двух эл-тов:
# головного и хвостового, а также текущую заполненность дека.
#
...
#
# Смысл значений индекса головы и хвоста немного различается:
...
#
# В некоторые моменты работы дека указатели уменьшаясь могут получить
# отрицательные значения или увеливаясь - превысить максимальный индекс
# использованной базовой структуры типа список. Для перенаправления индексов
# при выходе за границы индексов списка используется приём закольцовки.
# Наглядно пример был разобран в теории курса:
# https://practicum.yandex.ru/learn/algorithms/courses/7f101a83-9539-4599-b6e8-8645c3f31fad/sprints/21360/topics/3fe165ac-9a25-44db-b5bf-106709d1c140/lessons/b5036361-6d27-45be-ac0a-3946d9a0fcde/
# В данной реализации для расчёта индексов используется метод __calc_idx.
#
# -- ДОКАЗАТЕЛЬСТВО КОРРЕКТНОСТИ --
# Структура связного списка не использована.
# Превышение размера и попытка извлечения из пустого дека обрабатываются.
# Добавление эл-тов производится в соседнюю с текущей позицией в сторону
# расширения с пересчётом контрольных указателей.
# Извлечение эл-тов также сопровождается пересчётом индексов, уже в сторону
# "сжатия".
#
# -- ОЦЕНКА СЛОЖНОСТИ --
# В моей реализации для добавления и извлечения эл-та
# используется по 4 операции. Все они имеют как пространственную,

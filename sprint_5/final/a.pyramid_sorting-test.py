# https://contest.yandex.ru/contest/24810/run-report/68154634/
...
#
# -- ОЦЕНКА СЛОЖНОСТИ --
# Временная сложность операции добавления или удаления эл-та в бинарную кучу
# в худшем случае составляет O(log N), где N - текущий размер кучи,
# а логарифм по основанию 2. Визуально logN определяет высоту кучи, а процесс
# добавления/удаления эл-та в худшем случае выглядит как последовательное
# прохождение по всем уровням кучи с одного крайнего на противоположный.
# Следовательно, временная сложность отдельных этапов пирамидальной сортировки
# будет O(n * log n), где n - количество эл-ов, которые надо отсортировать.
# Соответственно, сложность временная алгоритма всей задачи в целом равна
# O(n * log n) + O(n * log n), или O(2 * n * log n), или O(n * log n).
...
class Pyramyd:
    ...
    def __sift_up(self, idx):
        ...
        Args:
            idx (int): индекс, с которого начинать процесс.
        В условном цикле:
        Вычислить индекс "родителя", сравнить оба значения.
        Если свойства кучи нарушены, поменять местами, перейти к следующей
        итерации, иначе выход.
        """
        if idx == 1:
            return
        while idx != 1:
    ...
    def __sift_down(self, idx):
        ...
        Args:
            idx (int): индекс, с которого начинать процесс.
        В цикле:
        По индексу вычислить "потомков". Если их нет, выход.
        Из потомка(-ов) и текущего меньший должен встать на место
        текущего. Если меньший уже на месте, выход.
        Для следующей итерации задать стартовый индекс, тот, куда
        переместился эл-т на текущей итерации.
        """
        while True:
            left = 2 * idx
            right = left + 1
            ...
            if self.__items[idx] > self.__items[index_smallest]:
                self.__items[idx], self.__items[index_smallest] = \
                    self.__items[index_smallest], self.__items[idx]

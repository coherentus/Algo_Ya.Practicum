# https://contest.yandex.ru/contest/25070/run-report/69523532/
...
# стандартном списке Python. Эл-т списка - список Python, индекс эл-та - это
# номер вершины, которую описывает список. В списке номера смежных вершин.
...
def input_line(index, town_arr, in_line):
    """Занести данные в список смежности.

    Args:
        index (int:): номер вершины, для которой переданы данные
        town_arr (list[list[int:]]): список вершин, размер больше на 1,
    ...
    вершины к большему. Вводятся добавлением номера вершины в список,
    являющийся эл-том списка с индексом index.
    Значение 'R' - обратным, т.е. от большего номера вершины к меньшему.
    Вводятся добавлением номера вершины в список,
    являющийся эл-том списка с индексом, вычисляемым из переданного
    ...
    for char_num, char in chars_in_line:
        if char == SYMB_B:
            town_arr[index].append(char_num)
        else:
            town_arr[char_num].append(index)
...
def check_optimum(towns_arr):
    ...
    Ориентированный граф передан списком смежности. По индексу эл-та список
...
def main():
    ...
    with open('input.txt') as file_in:
        ...
        # cписок смежности в виде списка списков.
        # towns[NN] - список вершин, смежных NN.
        towns = [list() for _ in range(num_towns + 1)]
